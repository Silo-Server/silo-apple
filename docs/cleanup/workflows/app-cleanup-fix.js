...
- Xcode DerivedData lives at /Volumes/NVMe/DeveloperCache/xcode-derived-data (already configured); a cold build takes several minutes — that is expected, wait for it. The iOS test suite (~1539 tests) takes ~10 minutes including simulator boot.
- BASELINE: since 2026-08-18 the suite is genuinely green — "Executed 1539 tests, with 3 tests skipped and 0 failures" (1520 on 2026-08-18; 19 tests added since by the PR-review, telemetry and auth-guard layers) on this host (the 3 skips are keychain-migration tests when the sim host cannot write the keychain; the old 14 environment failures were fixed via an injected in-memory keychain backend + stale-expectation updates). ANY failure is yours to fix; a change in the total count you can't explain by tests you removed/added is a regression. Totals drift by host — take a same-host control at the base SHA before reading a delta.
`
