export const meta = {
  name: 'app-cleanup-review-continue',
  description: 'Finish the interrupted round-6 app-cleanup-fix run (wf_b6f0ac5c-dd1): independent Opus review for the packages whose review never ran, one repair round + re-review',
  phases: [
...
// args: { baseRef: <40-char SHA>, scratchDir, packages: [ { id, title, area, risk, files, build_scopes, effort, spec_path } ],
//         fixes: { <id>: <FIX_RESULT_SCHEMA object from the interrupted run's journal.jsonl or run record> } }
// The round-6 run embedded these inline (const EMBEDDED_ARGS = {...}) because the packages were too large to pass as tool args.
const EMBEDDED_ARGS = null
const A = (args && args.packages) ? args : EMBEDDED_ARGS
if (!A || !Array.isArray(A.packages) || !A.fixes) throw new Error('args.packages and args.fixes are required (see header comment)')
const PACKAGES = A.packages
const FIXES = A.fixes
const BASE_REF = A.baseRef
const FIXER_MODEL = A.fixerModel || 'opus'
const REVIEWER_MODEL = A.reviewerModel || 'opus'
const RUN_TESTS = A.runTests !== false
const BRANCH_PREFIX = 'cleanup'
if (!/^[0-9a-f]{40}$/.test(BASE_REF)) throw new Error('baseRef must be a full SHA')
const SCRATCH = A.scratchDir
...
- Xcode DerivedData lives at /Volumes/NVMe/DeveloperCache/xcode-derived-data (already configured); a cold build takes several minutes — that is expected, wait for it. The iOS test suite (~1538 tests) takes ~10 minutes including simulator boot.
- BASELINE: since 2026-08-18 the suite is genuinely green — "Executed 1538 tests, with 3 tests skipped and 0 failures" (1520 before the round-6 base 034b4df added tests) on this host (the 3 skips are keychain-migration tests when the sim host cannot write the keychain; the old 14 environment failures were fixed via an injected in-memory keychain backend + stale-expectation updates). ANY failure is yours to fix; a change in the total count you can't explain by tests you removed/added is a regression. Totals drift by host — take a same-host control at the base SHA before reading a delta.
`
...
const SIM_TEMPLATE_UDID = (A && A.simTemplateUdid) || '89473B29-E0D9-4FB2-83DA-D16856E3BC70' // iPhone 17 Pro, iOS 26.5
function testCmd(id) {
...
// ---------------------------------------------------------------------------
// Pipeline: review -> (repair -> re-review)
// ---------------------------------------------------------------------------
phase('Review')
const results = await pipeline(
  PACKAGES,
  pkg => {
    const fix = FIXES[pkg.id]
    if (!fix) return { pkg, fix: null, review: null, error: 'no implementer report' }
    return agent(reviewerPrompt(pkg, fix), { label: `review:${pkg.id}`, phase: 'Review', schema: REVIEW_SCHEMA, model: REVIEWER_MODEL, ...(pkg.effort ? { effort: pkg.effort } : {}) })
      .then(review => ({ pkg, fix, review }))
  },
  async r => {
    ...
    if (!repaired) return { ...r, repair: null, error: 'repair agent returned nothing' }
...
const summary = results.filter(Boolean).map(r => {
  ...
  return {
    ...
    repair_status: r.repair ? r.repair.status : null,
    first_review_verdict: r.review ? r.review.verdict : null,
  }
})
const approved = summary.filter(s => s.verdict === 'approve').map(s => s.id)
log(`Review-continue: ${approved.length}/${summary.length} approved`)
