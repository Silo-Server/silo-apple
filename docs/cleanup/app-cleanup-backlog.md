...
| **Stage 2 — wave 3 (reducer + actor cutover, 2026-08-19/20; `player-stage2-fix.js` + two orchestrated extra rounds, 1 Opus implementer (max) + 3 independent Opus review rounds + one orchestrator-applied minor)** | The hard cutover's centrepiece: `PlaybackSessionActor` runs `PlaybackReducer` and owns every control-plane effect (session start/stop, engine load/dispose with `SourceCacheDisposition`, seek, replan/renewals, recovery execution, timers via `PlayerTaskRegistry`, coalesced publishes); `PlayerViewModel` is a presentation model — `beginFreshLoad`/`loadStream`/`adoptPreparedPlayback`/`attemptProtocolV3Replan`/the renewal+outage execution bodies/`finalizeTerminalPlaybackError`/`consumeEngineEvents`/`executeRecoveryAction` and every remaining identity mirror (`activePlaybackSessionId`, the generation counters, both renewal session-id echoes) are **deleted** (greps 0); engine events are consumed by the actor and `EngineEvent.recoveryAction` becomes `PlayerEvent.recovery`. The two wave-2b disclosed gaps closed as legacy restorations (actor-scoped ride-through carry; post-outage suppression window back to the fileLoaded release). Round 1 review: 8 issues, all repaired, plus a blocker the repairer itself found — the visible server-outage recovery never reloaded (device row 13). Round 2 review: 2 new blockers — the playhead **axis mismatch** (raw player time stored as movie time; would have corrupted wire-visible watch progress on offset routes) and startup engine failures **swallowed on the V3 path** (every ladder arm required `.playing`) — plus `.retainProxy` retaining nothing and 3 minors. Round-2 repair fixed all with rung-for-rung base traces (`promotedForRecovery`; one axis conversion site in the engine session; `SuspendedContext.retainedLoadID`), 10 new tests. Round 3: **approve except one minor** (the pause publish still derived the overlay from state, missing the tvOS interruption-resume case) — applied by the orchestrator as `Presentation.isLoading: Bool?` with nil = leave-alone, predicate deleted, pinned. As-built contract recorded in design §2.3 for wave 4. | 18 files, +4,014 / −2,044 raw; 39 new tests across actor/reducer/session suites; suite **1,795 / 3 skipped / 0** at the branch head and re-verified on the merged tip, all three schemes green; §6 rows 13/14 (the blockers' device paths) queued for the post-wave-3 full gate |
| **Stage 2 — wave 4 (TrackSelection model + deletions + docs truth pass, 2026-08-20; `player-stage2-fix.js`, 2 file-disjoint packages, 2 Opus implementers + 2 independent Opus reviewers — BOTH APPROVED FIRST PASS, no repair round)** | **4A** `TrackSelection` (Tracks/TrackSelection.swift: `audio: AudioSelection`, `primary`/`secondary: SubtitleSelection`, `origin: SelectionOrigin`, each case naming its index space; single-rung values stored bare, ladders only via the setters) replaces the eight `pending*` optionals, `hasExplicitSubtitleChoice` (now `origin == .user` at all 9 gates) and the `pendingRecovered*` snapshots on `TrackSelectionCoordinator`; `ProtocolV3SidecarRestoreIntent` deleted (`SubtitleSelection.planned`/`sidecarRestore` reproduce it); planner/prefs/VM callers retargeted label-only; the nine reason strings and decision-block order byte-identical (reviewer diffed every site); 9 `TrackSelectionTests` + the three A.5 `pending*`-shaped `PlaybackProtocolV3Tests` rewritten against the model (rows 1-3 already reducer-shaped after wave 3). **4B** `[CMP-MEM]` block and the 8×200 ms initial-seek retry deleted from `AVPlayerBackend` (the `SeekRequest` deadline covers the deadline branch — design §7.5; the reviewer notes the mis-landed-seek branch now starts playback at the landing instead of retrying paused, a control-plane tightening candidate, no change required), `SILO_KEEP_DV_HLS` confinement verified; `docs/tvos-player` 01-09 + README + 10-playback-continuity truth pass to the as-built ownership (actor / engine session / LocalHLSHost / RecoveryPolicy+Driver / coordinator), validation records linked. Reviewer minors applied by the orchestrator post-merge: the dead `proxyStatsProvider` requirement removed end-to-end (protocol, backend, session wiring, fake, test); `planned()` refuses a negative plan index; secondary-axis carrier comments; one over-claiming docs sentence scoped. Deferred: structural enforcement of the ladder invariant (comment-only today). | 23 files, +1,284 / −476 raw; suite **1,804 / 3 skipped / 0** (1795 + 9) on 4A's head vs a same-host control of 1,795 at base, 4B neutral; merged tip + minors re-verified, all three schemes green |
