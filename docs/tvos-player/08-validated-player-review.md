...
> **Historical document. Superseded in part by the one-player consolidation on
> 2026-08-16.**
>
> This review was written against a two-backend stack. `PlayerCore` /
> CompatibilityPlayer was deleted on 2026-08-16 (`f1b1bba`), so every finding
> that names it — its audio graph, its locks, its PiP plumbing, its route
> priorities, and the escalation paths that fell back to it — **is moot**. Those
> rows are annotated **[MOOT 2026-08-16]** below rather than deleted, so the
> reasoning stays readable and so nobody re-files a fix against a file that no
> longer exists.
>
> Two more renames postdate this document and are not annotated row by row:
> `DVSegmentWriter` / `DVSegmentStore` / `DVSegmentServer` are now
> `LoopbackSegmentWriter` / `LoopbackSegmentStore` / `LoopbackSegmentServer`,
> and `avPlayerLocalDVLoopback` is now `siloPlayerLoopback`. Several
> Silo-priority items here (byte ranges, HEAD, segment eviction, mux/file-write
> error propagation) have since been implemented — check the code before acting
> on them.
>
...
> [02](02-retired-compatibility-player.md),
> [05](05-route-capability-matrix.md), and [09](09-video-bridge.md).
...
- **SiloPlayer** currently means `avPlayerLocalDVLoopback`: local FFmpeg remux,
  ...
  playback.
- ~~**CompatibilityPlayer** means `playerCoreDirect`~~ **[MOOT 2026-08-16 — backend deleted]**: FFmpeg demux/decode helpers,
...
1. ~~Fix stale docs and capability wording around CompatibilityPlayer audio and
   route-limited subtitle behavior.~~ **[MOOT 2026-08-16 — the route is gone;
   the docs pass landed with the consolidation.]**
...
| SiloPlayer is the local Dolby Vision loopback implementation. | **Superseded 2026-08-16** — SiloPlayer is now the *primary* direct-play route for H.264/HEVC/DV **and** the bridged codec tail, not a DV-only path. Confirmed, narrowed at the time | [`DVSegmentWriter.swift`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/DVSegmentWriter.swift), [`DVSegmentServer.swift`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/DVSegmentServer.swift), [`ISOBoxSurgery.swift`](../../iosApp/iosApp/Screens/Player/AVPlayerRoute/ISOBoxSurgery.swift) | Silo is not a general "all non-native remux" path. H.264 and SDR HEVC loopback are explicitly blocked back toward CompatibilityPlayer in the planner. |
| **[MOOT 2026-08-16]** ~~CompatibilityPlayer is the owned FFmpeg/CoreMedia path.~~ | Moot — deleted in `f1b1bba` | [`PlayerCore.swift`](../../iosApp/iosApp/Screens/Player/CoreMedia/PlayerCore.swift), [`PlayerSurface.swift`](../../iosApp/iosApp/Screens/Player/CoreMedia/PlayerSurface.swift) | VideoToolbox is the normal video path, but H.264 can switch to software decode after VT failures. |
...
| --- | --- | --- |
| **[MOOT 2026-08-16]** ~~[`02-coremedia-pipeline.md:15, 117`](./02-retired-compatibility-player.md) and [`README.md:38`](./README.md) still say CompatibilityPlayer audio uses `AVSampleBufferAudioRenderer`.~~ (File renamed to `02-retired-compatibility-player.md`.) Current code uses `AVAudioEngine` ([`PlayerCore.swift:74`](../../iosApp/iosApp/Screens/Player/CoreMedia/PlayerCore.swift#L74)) plus `AVAudioSourceNode`. | Confirmed | Update both docs to describe `AVAudioEngine` output. |
...
| **[MOOT 2026-08-16]** ~~`Thread.sleep(0.002)` polling exists on the video feed path.~~ | Moot — `PlayerCore` video feed deleted | Low | [`PlayerCore.swift:3944`](../../iosApp/iosApp/Screens/Player/CoreMedia/PlayerCore.swift#L3944) | Replace with the existing wait/broadcast style used elsewhere when touching this path. |
...
| **[MOOT 2026-08-16]** ~~Existing PiP plumbing is disabled sample-buffer PiP in `PlayerCore`.~~ | Moot — the recommendation was followed: iOS PiP is now AVPlayer-route work in `PictureInPictureCoordinator`. Narrowed at the time ([`PlayerCore.swift:1336-1339`](../../iosApp/iosApp/Screens/Player/CoreMedia/PlayerCore.swift#L1336) early-returns the PiP setup; uses `AVSampleBufferDisplayLayer`) | Treat Native/Silo PiP as separate AVPlayer-route work, not a free reuse of current `PlayerCore` PiP code. |
...
| **[SUPERSEDED 2026-08-16]** H.264 and SDR HEVC loopback branches are intentionally blocked in normal planner selection. | Resolved — the static VOD serving mode lifted both. `h264_loopback_startup_unreliable` and `hevc_sdr_loopback_startup_unreliable` were deleted outright on 2026-08-17 when the EVENT serving mode and its kill switch were retired. Confirmed, narrowed at the time ([`ApplePlaybackRoutePlanner.swift:518-530`](../../iosApp/iosApp/Screens/Player/ApplePlaybackRoutePlanner.swift#L518) — `passthroughH264` returns `blockedSilo` with sentinel `h264_loopback_startup_unreliable`; commit `f59b7bc` restored this guard) | Keep or comment these as sentinels if backend support remains. They are not globally impossible, but normal route planning blocks them. |
...
| **[MOOT 2026-08-16]** ~~CompatibilityPlayer is the closest area to KSPlayer's FFmpeg/VT path.~~ | Moot — no FFmpeg/VT decode path remains to compare. The loopback's [video bridge](09-video-bridge.md) is the nearest surviving analogue. | Audio strategy, Metal rendering, bitmap subtitles, scrubber thumbnails, and buffer/ABR ideas belong here if copied conceptually. |
| **[MOOT 2026-08-16]** ~~Typed `StreamRejection` handoff exists.~~ | Moot — decode-time `StreamRejection` recovery was retired with `PlayerCore`; signalled DV is routed up front by the planner. Was: P5 and HEVC HDR/PQ can build loopback plans; H.264 bad-data remains terminal. |
...
- Do not say `PlaybackSourceProxy` and H.264 software fallback have zero docs
  ...
  coverage.
- **[MOOT 2026-08-16]** ~~Do not treat existing `PlayerCore` PiP plumbing as something that
  benefits NativePlayer/SiloPlayer without AVPlayer-route PiP work.~~ (Moot;
  the AVPlayer-route PiP work was done.)
- Do not keep the `onSidecarTracksRegistered` disposed-state finding. It is
  already guarded.

## Recommended Implementation Order

1. **[MOOT 2026-08-16]** ~~Documentation truth pass: CompatibilityPlayer audio output, route
   capability matrix wording, Profile 7 HDR10 fallback setting,
   source-proxy/backpressure.~~ (Done, and redone for the one-player stack.)
2. Silo error propagation: mux write failures, file write failures, localhost
   server startup behavior.
3. **[MOOT 2026-08-16]** ~~CompatibilityPlayer audio failure propagation and audio callback
   lock cleanup.~~
...
**[MOOT 2026-08-16]** A `SiloTests` target exists in
[`iosApp/project.yml`](../../iosApp/project.yml), with the suites under
[`iosApp/Tests/`](../../iosApp/Tests) — including the route-planner pin tests and
the video-bridge planner/writer suites this section wished for. Original text
follows.
