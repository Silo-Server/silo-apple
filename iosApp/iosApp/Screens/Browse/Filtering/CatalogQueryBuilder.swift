...
/// Builds the v2 catalog query for a `CatalogFilterState`.
///
/// Multi-value facets become one structured group per facet (`match=any`
/// inside, one rule per value); the top-level `match` controls AND/OR across
/// facets. Rule values carry their JSON type: the server compares booleans
/// (`watched`, `hdr`, …) and year ranges as JSON booleans and numbers, not
/// strings. `SiloAPI.catalogPage` sends the groups as the `groups` JSON
/// parameter, or in a `POST /catalog/query` body when they are too long.
...
private struct GroupAccumulator {
    ...
    /// Year bounds are numbers and flag fields are booleans; every other
    /// facet compares a single string.
    func encoded() -> [APIv2CatalogGroup] {
        groups.map { group in
            APIv2CatalogGroup(match: group.match, rules: group.rules.map { rule in
                ...
                } else if Self.booleanFields.contains(rule.field) {
    ...
    private static let booleanFields: Set<String> = [
        "hdr", "dolby_vision", "watched", "in_progress", "favorited", "in_watchlist",
    ]
}
