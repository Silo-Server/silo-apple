...
/// (e.g. /discover → SectionsResponse), so types stay at the call site.
///
...
actor SiloAPI {
    ...
    // spin-up), so the start / replan calls opt out of the fail-fast timeout.
    // A dead server is still caught early: connection-refused fails instantly,
    // and the detail screen's own fetches run on the standard timeout.
