...
struct SubtitleUrl: Codable, Identifiable, Hashable {
    ...
    /// loopback and native-direct routes, or a server burn-in on any route.
    /// They can never be drawn by the text (libass) sidecar route, so a `.sup`
    /// sidecar must not be treated as a text sidecar.
    var isBitmapCodec: Bool { SubtitleSelection.isBitmapCodec(codec) }
}
