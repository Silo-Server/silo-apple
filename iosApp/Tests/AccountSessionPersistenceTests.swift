...
final class AccountSessionPersistenceTests: XCTestCase {
    ...
    func testFailedInstallNeverPublishesOrFallsBackToLegacyMirrors() async throws {
        ...
        do { try await store.installAccountSession(accessToken: "new", refreshToken: "new-refresh", accountID: "12"); XCTFail() } catch {}
    ...
    func testTemporaryCredentialsCannotReplaceDurableBinding() async throws {
        ...
        XCTAssertTrue(saved)
        do { try await store.installAccountSession(accessToken: "wrong", refreshToken: "wrong-refresh", accountID: "99"); XCTFail() } catch {}
